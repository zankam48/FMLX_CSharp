...
//             else
//                 Console.WriteLine("Game Over! No winner determined.");
...
//             bool extraTurn = ExecuteTurnForPlayer(currentPlayer);
//             if (!extraTurn)
//             {
//                 NextPlayerTurn();
//             }
...
//         // Executes a turn for a given player.
//         // Returns true if the turn ended with a six (extra turn awarded), false otherwise.
//         private bool ExecuteTurnForPlayer(Player currentPlayer)
//         {
//             display.DisplayBoard();
...
//             OnSixRoll?.Invoke(player, piece, rollResult);

//             Console.WriteLine("You rolled a six! You get an extra turn.");

//             // If player has pieces at home, offer to bring one out.
...
//                     if (movablePieces.Count > 0)
//                     {
//                         Piece selectedPiece = SelectPiece(player, movablePieces);
//                         if (selectedPiece != null)
//                         {
//                             MovePiece(selectedPiece, rollResult);
//                             display.DisplayBoard();
...
//                     else
//                     {
//                         Console.WriteLine("No movable pieces available.");
//                     }
//                 }
//             }
//             else
//             {
//                 // If no pieces at home, must move one already in play.
//                 List<Piece> movablePieces = board.GetMovablePieces(player, rollResult)
//                     .Where(p => p.Status == PieceStatus.IN_PLAY)
//                     .ToList();
...
//             // Extra turn: call ExecuteTurnForPlayer again for the same player.
//             Console.WriteLine("Extra turn! Press any key to roll again...");
//             Console.ReadKey();
//             bool extraTurn = ExecuteTurnForPlayer(player);
//             if (!extraTurn)
//             {
//                 // After extra turn without another six, move on.
//                 // (Do nothing here; the main loop will call NextPlayerTurn.)
//             }
//         }
...
//             foreach (Player player in players)
//             {
//                 if (player.Score >= 4) // all four pieces reached the goal
//                 {
//                     isGameOver = true;
//                     break;
//                 }
...
//         {
//             return players.FirstOrDefault(p => p.Score >= 4);
//         }
...
//         public void AddPiece(Piece piece)
//         {
//             // Add piece to the corresponding player's collection and place it at home.
//             Player player = players.FirstOrDefault(p => p.Color == piece.Color);
//             if (player != null)
//             {
...
//         {
//             // Create board, players, dice, display, and game controller
//             Board board = new Board();
//             List<Player> players = new List<Player>()
//             {
//                 new Player("Alice", PieceColor.RED),
//                 new Player("Bob", PieceColor.GREEN),
//                 new Player("Charlie", PieceColor.YELLOW),
//                 new Player("Diana", PieceColor.BLUE)
//             };
//             Dice dice = new Dice();
//             Display display = new Display(board, players);
//             GameController gameController = new GameController(players, board, dice, display);

//             // Optionally, subscribe to events (if needed)
//             gameController.OnNextPlayerTurn += (player) => Console.WriteLine($"Next turn: {player.Name} ({player.Color})");
//             gameController.OnDiceRoll += (d) => d.Roll();
//             gameController.OnSixRoll += (player, piece, roll) => { /* Custom logic can be added here */ };
